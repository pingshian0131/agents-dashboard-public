...
src/                  # React 前端
  main.tsx            # 入口
  App.tsx             # 兩層導航（Dashboard + MemoryPage）
  api.ts              # 前端 API client
  types.ts            # 共用型別（Dashboard, MemoryPage, Skill 等）
...
- **頂層**: `Dashboard` 型別 — `'memory'` \| `'skills'`（Header 頂層列切換）
- **Memory sub-tabs**: `MemoryPage` 型別 — `'lancedb'` \| `'workspaces'` \| `'stats'`
